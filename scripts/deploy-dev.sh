...
DOCKER_IMAGE=docker.vgasoft.vn/thiethai-api-"$BUILD_TAG":"$BUILD_NUMBER"
ENV_FILE=/data/env/apithiethai/.env.production # Default to dev environment
CONTAINER_NAME=thiethai-api-"$BUILD_TAG"

if [ "$BUILD_TAG" = "main" ]; then
  ENV_FILE=/data/env/apithiethai/.env.production # Production environment file
elif [ "$BUILD_TAG" = "test" ]; then
  ENV_FILE=/data/env/apithiethai/.env.production # Staging environment file
fi
