...
import requestGroupsService from "../services/requestGroupsService";
...
export default {
  get_list: (req, res, next) => {
    ...
    try {
      ...
      requestGroupsService.get_list(param).then(data => {
        const dataOutput = {
          result: {
            list: data.rows,
            pagination: {
  ...
  get_one: (req, res, next) => {
    ...
    try {
      ...
      requestGroupsService.get_one(param).then(data => {
        res.send(data);
        recordStartTime.call(res);
        loggerHelpers.logInfor(req, res, {
          dataParam: req.params,
  ...
  create: (req, res, next) => {
    ...
    try {
      ...
      entity.userCreatorsId = req.auth.userId;
      const param = {entity}

      requestGroupsService.create(param).then(data => {
        if (data && data.result) {
          const dataOutput = {
            result: data.result,
            success: true,
