...
import humanDamagesService from '../services/humanDamagesService';
...
export default {
  ...
  bulk_create_t1: (req, res, next) => {
    recordStartTime.call(req);
    try {
      console.log('Request-Body create:', res.locals.body);
      const entity = res.locals.body;

      entity.usersCreatorId = req.auth.userId;
      const param = { entity };


      humanDamagesService
        .bulk_create_t1(param)
        .then(data => {
          if (data && data.result) {
            const dataOutput = {
              result: data.result,
