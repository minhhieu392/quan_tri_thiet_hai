...
const { chatbotTemplates, chatbotBlocks, chatbotCards, socialChannels, clinicQueues } = models;
    ...
    if (text) {
      ...
      if (
        ...
        const foundChatbotBlock = await Model.findOne(chatbotBlocks, {
          whereFilter = await filterHelpers.makeStringFilterRelatively(['intent'], whereFilter,'');
      ...
      if (typeof currentSession === 'object') {
        ...
        if (currentSession.preQuestion && currentSession.cards && currentSession.cards.length === 0) {
          ...
          if (currentSession.preQuestion['chatbotRegexsId']) {
            ...
            if (await checkValidate(text, currentSession.preQuestion['chatbotRegexsId'])) {
              ...
              finalResult.message = 'VAO TH HET QUESTION';
              ...
              finalResult.message = 'checkValidate';
            ...
            if (currentSession.preQuestion.hasOwnProperty('quickReplies')) {
              ...
              if (btnBlock) {
                ...
                console.log('currentSession quickReplies:', currentSession);
              } else {
        ...
        while (currentSession.cards && currentSession.cards.length > 0 && currentSession.cards[0]) {
          ...
          if (
            ...
            currentSession.preQuestion.hasOwnProperty('key') &&
            !currentSession.cards[0].hasOwnProperty('key') &&
            !currentSession.cards[0].hasOwnProperty('url')
          ) {
            if (currentSession.preQuestion['chatbotRegexsId']) {
          ...
          if (
            currentSession.cards[0].hasOwnProperty('key') &&
            !currentSession.cards[0].hasOwnProperty('quickReplies') &&
            !currentSession.cards[0].hasOwnProperty('redirectToBlock') &&
            !currentSession.cards[0].hasOwnProperty('url')
          ) {
            ...
            await Telegram.sendFile({
              accessToken: foundSocialChannel.token,
              userId: sender,
              path: currentSession.cards[0]['path']
            });
            currentSession.cards.shift();
            ...
            await Telegram.sendMessage({
                ...
                userId: sender,
                text: 'Mời lựa chọn: ',
                keyboard: Telegram.convertKeyboard2D(currentSession.cards[0]['quickReplies'].map(e => e.buttonName))
              })
            ...
            if (currentSession.cards[0].url === CONFIG.CLINIC_QUEUES_API) {
              ...
              if (method === 'post') {
                ...
                if (
                  ...
                  console.log('currentSession: ', currentSession);
                  // eslint-disable-next-line no-await-in-loop
                  await clinicQueueService.send_quick_replies_service_telegram(
                  ...
                  if (currentSession.preQuestion['chatbotRegexsId']) {
                    ...
                    if (await checkValidate(text, currentSession.preQuestion['chatbotRegexsId']) || text === 'xemthem' || text === 'trolai') {
                      ...
                      if (currentSession.preQuestion.key === 'servicesId') {
                        ...
                        // eslint-disable-next-line no-await-in-loop
                        await clinicQueueService.send_quick_replies_service_package_telegram(
                          foundSocialChannel.placesId,
                        ...
                        currentSession.preQuestion = questionGPP[parseInt(currentSession.preQuestion.index) + 1]; // tiep tuc cau hoi tiep theo
                        // eslint-disable-next-line no-await-in-loop
                        await clinicQueueService.send_date_scheduled_telegram(
                        ...
                        currentSession.preQuestion = questionGPP[parseInt(currentSession.preQuestion.index) + 1]; // tiep tuc cau hoi tiep theo
                        // eslint-disable-next-line no-await-in-loop
                        const rely = await clinicQueueService.send_time_scheduled_telegram(
                          date,
                        ...
                        if (text === 'xemthem') {
                          ...
                          await Telegram.sendMessage({
                              ...
                              keyboard: Telegram.convertKeyboard2D(rely)
                            })
                            .then(async dataQuick => {
                              console.log('dataQuick: ', dataQuick);
                              currentSession = await redisService.setAndGetCurrentSession({
                ...
                // eslint-disable-next-line no-await-in-loop
                await clinicQueueService.check_schedule_queue_telegram(
                  date,
                  mobile,
                  foundSocialChannel.placesId,
                  foundSocialChannel.token,
                ...
                if (text === 'xemthem') {
                  ...
                  // eslint-disable-next-line no-await-in-loop
                  const rely = await clinicQueueService.update_schedule_queue_telegram(
                    date,
                    foundSocialChannel.placesId,
                    foundSocialChannel.token,
                    sender,
