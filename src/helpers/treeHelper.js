...
export default {
  ...
  createTree: (data = [], currentKey = 'id', parentKey = 'parentId', firstParentValue = 0) => {
    ...
    const dataObjectResult = {}; // object check key
    const dataResult = [];
    ...
    data.forEach(dataElement => {
      ...
      if (
        !dataObjectResult[dataElement[currentKey]] || // chưa lưu vào dataObjectResult
        (dataObjectResult[dataElement[currentKey]] && dataObjectResult[dataElement[currentKey]].created) //  chưa tìm thấy cấp cha của nó ( nhưng không phải cấp cao nhất)
      ) {
        if (!dataObjectResult[dataElement[currentKey]]) {
          //  lưu vào dataObjectResult
          dataObjectResult[dataElement[currentKey]] = dataElement;
        } else {
          // đã tồn tại trong dataObjectResult => marge thông tin
          dataElement.children = dataObjectResult[dataElement[currentKey]].children;
          dataObjectResult[dataElement[currentKey]] = dataElement;
        }

        // xác định cấp cao nhất, lưu vào kết quả
        if (Number(dataElement[parentKey]) === firstParentValue) {
          // cấp cao nhất của tree
          dataResult.push(dataElement);
        } else if (dataObjectResult[dataElement[parentKey]]) {
          //  tìm thấy cha của nó, push vào children
          dataObjectResult[dataElement[parentKey]].children.push(dataElement);
        } else {
          // không phải cấp cao nhất , nhưng hiện tại chưa tìm thấy cấp trên của nó
  ...
  createTreeCheckSelect: (
    ...
    data.forEach(dataElement => {
      ...
      if (
        ...
        // đánh dấu đã select
        if (selectName && dataElement[selectName]) {
          console.log('current True ', dataElement.id);
        ...
        if (!dataObjectResult[dataElement[currentKey]]) {
          ...
          dataElement.children = dataObjectResult[dataElement[currentKey]].children;
          dataElement.selected = dataElement.selected || dataObjectResult[dataElement[currentKey]].selected;
          dataObjectResult[dataElement[currentKey]] = dataElement;
        }
        ...
        if (Number(dataElement[parentKey]) === firstParentValue) {
          ...
          dataObjectResult[dataElement[parentKey]].children.push(dataElement);
          // select con => có select cha
          if (dataElement.selected === true) {
            dataObjectResult[dataElement[parentKey]].selected === true;
          }
        } else {
          // không phải cấp cao nhất , nhưng hiện tại chưa tìm thấy cấp trên của nó
          ...
          if (dataElement.selected === true) {
            dataObjectResult[dataElement[parentKey]].selected = true;
            console.log('parentTrue', dataElement[parentKey]);
          }
        }
      }
    });
    ...
    data.forEach(dataElement => {
      ...
      if (
        ...
        if (!dataObjectResult[dataElement[currentKey]]) {
          ...
          // đã tồn tại trong dataObjectResult => marge thông tin
          if (Number(dataElement.id) === 9) {
            console.log('dataElement', dataElement);
            console.log('dataElement', dataObjectResult[dataElement[currentKey]]);
          }

          (sumAttributesName || []).forEach(sumAttributesNameElement => {
            dataElement[sumAttributesNameElement] = dataObjectResult[dataElement[currentKey]][sumAttributesNameElement];
          });
          dataElement.children = dataObjectResult[dataElement[currentKey]].children;
          dataObjectResult[dataElement[currentKey]] = dataElement;
        }
        ...
        if (Number(dataElement[parentKey]) === firstParentValue) {
          ...
          // select con => có select cha
          //  tìm thấy cha của nó, push vào children
          console.log('1b', dataElement.id);
          (sumAttributesName || []).forEach(sumAttributesNameElement => {
            dataObjectResult[dataElement[parentKey]][sumAttributesNameElement] = dataObjectResult[
              dataElement[parentKey]
            ][sumAttributesNameElement]
              ? dataObjectResult[dataElement[parentKey]][sumAttributesNameElement] +
                  dataElement[sumAttributesNameElement] || 0
              : dataElement[sumAttributesNameElement] || 0;
          });
        } else {
          // không phải cấp cao nhất , nhưng hiện tại chưa tìm thấy cấp trên của nó
          ...
          console.log('1a', dataElement.id, dataObjectResult[dataElement[parentKey]]);
        }
      }
    });
