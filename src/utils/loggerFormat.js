...
export default async (req, res) => {
  ...
  if (responseTime > Number(CONFIG['SLACK_TIMEOUT_MIN_WARNING'] || 1000)) {
    ...
    //   type: 'mrkdwn',
    //   text: `*WARNING CHECKTIME ${responseTime} ms*\n*Datetime:* ${new Date()}\n*url:* ${JSON.stringify(
    //     req.url,
    //     null,
    //     2
 ...
 * @param {*} res
 */
export async function logTimeToSlack(req, res) {
  console.log('abc');
  const ip = getip(req);
  const method = req.method;
  const url = req.originalUrl || req.url;
  ...
  await Slack.sendMessage({
    channel: 'mybooking-notify',
    accessToken: 'xoxp-374681962228-375797693286-2881513592007-340c5341b1a8fb116ddfa623f910cd6f',
    type: 'mrkdwn',
    text: `*WARNING CHECKTIME ${responseTime} ms*\n*Datetime:* ${new Date()}\n*url:* ${JSON.stringify(
      req.url,
      null,
