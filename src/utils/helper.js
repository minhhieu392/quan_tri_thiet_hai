      ...
      typeof sort === 'string'
        ? JSON.parse(sort).map((e, i) =>
            i % 2 === 0
              ? e.includes('.')
                ? e.includes('.status') ||
                  e.includes('.createDate') ||
                  e.includes('.dateScheduled') ||
                  e.includes('.price') ||
                  e.includes('.dateCreate') ||
                  e.includes('.dealPrice') ||
                  e.includes('.orderBy')
                  ? sequelize.literal(`\`${e}\``)
                  : sequelize.literal(`\`${e}\``)
                : e.includes('status') ||
...
export const parseSortVer3 = sort => {
  let isPass = true;
  let strReturn = '';
  const defaultValue = 'id asc';
  const regexSort = /(asc|desc|ASC|DESC)/g;
  ...
  try {
    ...
    console.log('sort1===', typeof sort === 'string');
    // console.log("table===",table)

    _.forEach(sort, function(value, index) {
      if ((index + 1) % 2 === 0 && index !== 0) {
        if (_.size(sort) === index + 1) {
          strReturn += ' ' + value;
        } else {
          strReturn += ' ' + value + ',';
        }
