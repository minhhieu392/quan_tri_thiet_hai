...
const { disasters, requests, requestGroups, wards, districts, provinces } = models;
...
export default {
  get_list: param =>
    new Promise(async (resolve, reject) => {
      try {
        ...
        await MODELS.findAndCountAll(requests, {
          where: whereFilter,
          order: sort,
          attributes: att,
          offset: range[0],
    ...
    new Promise((resolve, reject) => {
      try {
        ...
        MODELS.findOne(requests, {
          ...
          .catch(err => {
            reject(ErrorHelpers.errorReject(err, 'getInfoError', 'requestService'));
          });
      } catch (err) {
        reject(ErrorHelpers.errorReject(err, 'getInfoError', 'requestService'));
      }
  ...
  create: async param => {
    ...
    try {
      ...
      finnalyResult = await MODELS.create(requests, entity).catch(error => {
      ...
      ErrorHelpers.errorThrow(error, 'crudError', 'requestService');
    }
  ...
  update: async param => {
    ...
    try {
      ...
      if (foundRequest) {
        ...
        finnalyResult = await MODELS.findOne(requests, { where: { id: param.id } }).catch(error => {
          throw new ApiErrors.BaseError({
            statusCode: 202,
            type: 'crudInfo',
  ...
  delete: async param => {
    ...
    try {
      ...
      if (foundRequest) {
        await MODELS.destroy(requests, { where: { id: Number(param.id) } }).catch(error => {
          throw new ApiErrors.BaseError({
            statusCode: 202,
            type: 'crudError',
            error
