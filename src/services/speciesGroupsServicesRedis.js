  ...
  get_list: param =>
    new Promise(async (resolve, reject) => {
      try {
        ...
        const getDatFromRedis = await myRedis.getWithoutModel({
          router: 'speciesGroups',
          infoFilter: {
            filter,
            range,
            sort,
        ...
        console.log('getDatFromRedis', getDatFromRedis);

        if (getDatFromRedis) {
          resolve({
            ...JSON.parse(getDatFromRedis),
            page: page + 1,
            perPage
          });
        } else {
  ...
  update: async param => {
    ...
    try {
      ...
      if (foundProvince) {
        ...
        console.log('b');
        // const keys = await myRedis.getKeyList({
        //   router: 'speciesGroups'
        // });

        // console.log('keys', keys);
        await myRedis.deleteKey({
          router: 'speciesGroups'
        });
      } else {
        throw new ApiErrors.BaseError({
              ...
              if (Number(attributeGroupsElement.flag) === 1) {
                if (Number(attributeGroupsElement.id) === 0) {
                  ...
                  result = await MODELS.findOne(attributeGroups, {
                    ...
                    //   'id',
                    //   'attributeGroupsName',
                    //   [sequelize.literal('CAST(`type` AS CHAR)'), 'type'],
                    //   [sequelize.literal('CAST(attributeGroups.`order` AS CHAR)'), 'order']
                    // ],
                    // include: [
                    //   {
                    //     model: attributes,
                    ...
                    //       'id',
                    //       'attributesName',
                    //       [sequelize.literal('CAST(`dataType` AS CHAR)'), 'dataType'],
                    //       [sequelize.literal('CAST(`viewType` AS CHAR)'), 'viewType'],
                    //       [sequelize.literal('CAST(`selectType` AS CHAR)'), 'selectType'],

                    //       [sequelize.literal('CAST(`positionType` AS CHAR)'), 'positionType'],
                    //       [sequelize.literal('CAST(`placeholder` AS CHAR)'), 'placeholder'],
                    //       [sequelize.literal('CAST(`filterStatus` AS CHAR)'), 'filterStatus'],
                    //       [sequelize.literal('CAST(attributes.`order` AS CHAR)'), 'order']
                    //     ],
                    //     include: [
                    //       {
                    //         model: attributeSuggestions,
                    //         as: 'attributeSuggestions',
                    //         attributes: [
                    //           'attributeSuggestionsName',
                    //           [sequelize.literal('CAST(`editValueStatus` AS CHAR)'), 'editValueStatus']
                    //         ]
                    //       }
                    //     ]
                    //   }
                            ...
                            if (attributesElement.attributeSuggestions) {
                              ...
                              oldAttributeSuggestions.forEach(oldAtt => {
                                ...
                                if (findAttributeSuggestions) {
                                  ...
                                  if (
                                    Number(oldAtt.editValueStatus) !== Number(findAttributeSuggestions.editValueStatus)
                                  ) {
